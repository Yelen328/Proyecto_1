...
ALARMA:
	...
	SBIS	PINC, PC0			//SI el bit 0 del PINC es 0 (No apachado)
	CALL	INC_CONT_ALARMA // Si está en 1 ejecuta esta línea
	
	RJMP	LLAMAR_SALIR_ALARMA

INC_CONT_ALARMA:
	LDS		R16, INDICADOR_ALARMA
	INC		R16
	CPI		R16, 1
	BREQ	ACTIVAR_ALARMA
	STS		INDICADOR_ALARMA, R16
	LDI		R16, 0x00
	STS		INC_CONT_ALARMA, R16
	RJMP	LLAMAR_SALIR_ALARMA
...
ACTIVAR_ALARMA:
	...
	LDS		R17, CONT_DHORA
	CP		R16, R17
	BRNE	SALIR_ACT_DHORA
	STS		CONT_DHORA_ALARMA, R16
	STS		CONT_DHORA, R17

	SBI		PORTB, PB2

ESPERAR_DESACTIVACION:
    SBIS    PINC, PC1           ; Si el botón de apagado está presionado
    CALL    APAGAR
    RJMP    SALIR_ALARMA
APAGAR:
...
/*	RJMP	APAGAR
	SBIS	PINC, PC1			//SI el bit 0 del PINC es 0 (No apachado)
	CALL	APAGAR				 // Si está en 1 ejecuta esta línea
	RJMP	SALIR_ALARMA
